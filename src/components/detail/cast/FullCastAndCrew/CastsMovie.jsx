...
const CastsMovie = ({ movie, state }) => {
  ...
  const currentItemsCastMovie = movie?.credits?.cast.slice(
    indexOfFirstItem,
    indexOfLastItem,
  );
  return (
    <>
      <div className="container my-20 pt-[850px]">
        <h1 className="mt-10 text-4xl font-bold text-white">
          Cast ({movie?.credits?.cast?.length})
        </h1>
        ...
        <div className="flex justify-center">
          <ReactPaginate
            className="my-14 flex w-fit justify-center gap-14 rounded-full bg-slate-900 px-10 py-3 text-white font-medium"
            pageCount={Math.round(movie?.credits?.crew?.length / perPage)}
